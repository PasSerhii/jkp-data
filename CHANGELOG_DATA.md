...
- Validated the August 31 snapshot replay: fresh WRDS history restores 212 SIC and 209 FF49 values; the preserved RDS audit extract restores all 213 SIC and 210 FF49 gaps, all matching SAS. Both retain all 46,867 rows and exactly preserve the other 453 columns. All 157 targeted tests pass. Fresh RDS connections timed out, so the RDS result is based on the saved audit extract, not a new full-universe RDS read. No full pipeline rebuild, deployment or database update was performed.

__Methodology decision (no runtime change)__:
- Retain `bkelly/main`'s `QUANTILE_DISC` percentile definition for factor breakpoints and NYSE size cutoffs, verified against upstream `666e896`. The existing calculations already match. Production SAS uses a different percentile definition that can average adjacent observations, so identical inputs can produce different breakpoints and portfolio assignments. Differences attributable to this definition are accepted; no SAS-percentile replacement is planned. This decision does not close other factor-input or coverage discrepancies.

## 22-09-2026
__Changes__:
- Restored `bkelly/main`'s Compustat package selection (upstream `666e896`): prefer Global when both packages supply the same annual `gvkey/datadate` or quarterly `gvkey/fyr/fyearq/fqtr` key, and retain periods supplied by only one package. Removed the 26-07-2026 completeness scoring and the fiscal-year vote initially introduced on 22-09-2026. The latter let later-quarter filings change earlier source selections and discarded newer quarters available only in the losing package. Global financial-statement definitions now take precedence over richer NA `INDL` records at overlapping periods; some capex and working-capital fields will therefore become null again. This accepts upstream's coverage tradeoff instead of selecting accounting concepts by their populated-field count.
- Prevented mixed-package quarterly flows in both arithmetic stages. `quarterize` no longer subtracts a Global year-to-date value from an NA value, or vice versa; directly reported quarterly values and each fiscal year's Q1 remain usable. `cumulate_4q` still requires the same package at all three lags and retains the Q4 year-to-date fallback. Global and NA encode semi-annual reporters differently, so combining their quarters had inflated BHP's August 2026 sales to 83,514. These guards prevent invalid differences and sums when single-package periods cause a source switch. Growth ratios across package definitions remain a known limitation; production discrepancy and coverage counts require a rerun.
- Fixed `sic`, `naics` and `ff49` going empty for the whole universe in every month after a company's latest fiscal year-end. `comp_sic_naics` expands each annual industry record into daily rows until the next record starts, but the newest record had no successor and was ended on its own `datadate` (one day of validity) — a faithful port of the production SAS (`ind_identification.sas`, `if FIRST.gvkey then valid_to = date` under a descending sort), which has the same defect and was patched the same way. Recent months had ~0% `sic` coverage in both systems and were being backfilled in the research database by hand after every upload. The newest record now runs through the run's `end_date`, as the GICS spans already did; a record dated after `end_date` keeps its single day. Recent months now carry ~93% `sic` coverage (the remainder has no Compustat industry record at all), which is what the manual as-of backfill produced (99.96% agreement with SAS-authored codes over 1.96M company-months when that rule was validated on 2026-09-02). `ff49`-based industry portfolios in `portfolios/` gain the same months.
- Fixed `ami_126d` (Amihud illiquidity) being nulled for any security with a zero-dollar-volume day inside the 126-day window. The production SAS divides day by day, so a zero-volume day is a missing ratio that `mean()` skips; the Python port additionally nulled the whole window whenever any day had zero volume. Compustat North America reports zero volume on no-trade days for illiquid names, so 689 US and Canadian securities (Air T, Bridgford Foods, Chicago Rivet, …) had no `ami_126d` in August 2026 while every other 126-day column of theirs matched the SAS exactly. Zero-volume days now drop out of the mean as in the SAS; the minimum-observation count is unchanged.

## 08-09-2026
__Changes__:
- Corrected the sign of the working-capital term in the Ohlson O-score: `o_score` now applies `-1.43 * wc` (more working capital lowers the distress score), matching Ohlson (1980) and the upstream SAS since its 2025-03-05 correction ([ReplicationCrisis@b0b01d5a](https://github.com/bkelly-lab/ReplicationCrisis/commit/b0b01d5a), recorded in this changelog under 05-03-2025 as "corrected error in o-score calculation"). The `+1.43` this port carried reproduced the pre-2025 SAS bug, which the WRDS production copy of `char_macros.sas` still contains — that copy should receive the same one-character fix, or SAS-vs-Python comparisons will differ on `o_score` and its descendants (`qmj_safety`, `qmj`).

## 31-08-2026
__Changes__:
- Fixed `world_ret_monthly` (both `return_data/world_ret_monthly.parquet` and `production/world_ret_monthly.csv`) exporting only the main-filtered universe. The production SAS (`save_monthly_ret_csv`) exports `scratch.world_msf` with **no screen** — the file includes non-common securities (ETFs, funds, preferred, warrants), secondary listings, and non-main-exchange observations — but the Python port read the `MAIN_FILTERS`-screened `world_msf_output` instead, dropping roughly half the rows (~53,200 of ~100,100 per month; e.g. June 2026: SAS 100,085 vs Python 46,856, with the Python rows a 99.95% strict subset of the SAS rows). A 1,000-row sample of the dropped ids classified against the Compustat security header shows 63% non-common (`tpci != '0'`: 45% ETFs, 10% funds, plus units/preferred/ADRs) and 37% common-and-primary rows excluded by `obs_main`/`exch_main` — exactly the filter population, confirming the mechanism. The export now reads the unfiltered `world_msf`, restoring the SAS universe. No values change on the rows both versions carry; the other outputs are unaffected (`characteristics/`, the production monthly/daily country CSVs, and the cutoff/market-return files are filtered identically in both systems).
- Fixed the delisting cut, which had not been firing since 23-07-2026. `gen_delist_df` gates the delisting truncation and the -0.30 delisting return on the Compustat vendor inactivation date (`dldtei`), so that a security which goes inactive *after* the downloaded window is not truncated early. That gate compared `dldtei` against **each security's own last observation**, but Compustat stamps `dldtei` *after* the final observation in 99.3% of delistings (732 of 737 sampled on WRDS `comp.security`/`g_security`; median 2 days in North America, 5 days globally), so the test could essentially never pass. The consequences were that delisted securities kept a final partial month that the production SAS removes, and — more materially — **the -0.30 delisting return applied to performance-related delistings (`dlrsni` in 02, 03) was never applied to any security**. Roughly 4,800 such delistings fall inside the 23-year source window. The comparison is now against the end of the panel, which restores both behaviours while still blocking the case the gate was written for: as of a 2026-07-31 panel, 82 securities have `dldtei` after the panel end and are correctly left untouched. In the June/July 2026 production comparison this accounted for 31 of the 42 securities present in the run but absent from `research`, and 586 of the 1,168 one-sided daily rows.
