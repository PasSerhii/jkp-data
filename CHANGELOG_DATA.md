...
## 22-09-2026
__Changes__:
- Fixed trailing-4-quarter accounting flows for companies filed in both the Compustat Global and North America packages. The completeness rule introduced on 26-07-2026 chose the package per fiscal *quarter*, and the two packages encode semi-annual reporters differently: Global halves each half-year flow across two quarters while NA carries the full half in fqtr 2 and 4 only. The choice therefore alternated GLOBAL/NA/GLOBAL/NA and `cumulate_4q` summed full halves with half-halves — BHP's August 2026 `sales` were 83,514 (26,232 + 13,951 + 27,902 + 15,429) against a reported 58,760, `net_income` 15,159.5 against 9,833; Prudential, Rio Tinto, NAB, DBS and ~250 other dual-filed companies (7.8% of world market cap, 36–78% of the GBR/FRA/DEU/NLD/ARG market caps) were affected, along with every characteristic built on `sale_x`, `ni_x` and the other quarterly flows. The quarterly package is now chosen once per fiscal year (`gvkey`, `fyr`, `fyearq`) by the summed populated count of its rows (tie → Global, as before), and `cumulate_4q` nulls any trailing window whose quarters come from different packages (the fqtr-4 fallback to the year-to-date value is unchanged). The annual panel keeps its per-`datadate` choice; a company whose package changes between fiscal years (190 such changes since 2020 across 170 companies) still has growth ratios that compare the two packages' definitions across that boundary — a known limitation left for a follow-up.
